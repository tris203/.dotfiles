...
function Buffer:render()
  ...
  -- strip formatting and calculate length
  local stripped_name = name:gsub('%%#[^#]+#', '')
  self.len = vim.fn.strchars(stripped_name)
  -- self.len = 12

  -- setup for mouse clicks
  local line = self:configure_mouse_click(name)
  -- apply highlight
  line = modules.highlight.component_format_highlight(self.highlights[(self.current and 'active' or 'inactive')]) .. line
